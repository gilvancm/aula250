...
using SalesWebMvc247.Models.Enums;
using System;
...
namespace SalesWebMvc247.Data
{
    ...
    {
        //registrar uma dependencia com a conecção DbContext

        private SalesWebMvc247Context _context;
        // recebe uma instancia (SalesWebMvc247Context context)
        public SeedingService(SalesWebMvc247Context context)
        {
            _context = context;
        }
        //Seed = semear ou seeding = semeador
        //operação responsavel de popular a minha base de dados
        public void Seed()
        {
            // primeiro eu testo se tem dados nas tabelas 
            //Any = verifica se existe registro na tabela, é uma função do linq
              ...
              )
            {
                ...
                return; // DB has been seded = O banco de dados já foi populado
            }

            //se passo no se criamos um objetos(os registros) e uma instancia
            ...
            //vamos adicionar estes objetos no banco de dados usando Entydframer
            ///AddRange   permite que vc adicione varios objetos de uma só vez 
            ...
            //terminanos a inclusão dos objetos no banco

            //SaveChanges() pra efetivar as alteraçoes no banco ele salva e confirmas as alterações no banco
